 ...
 *   “ранспонирование структуры матрицы
 * INPUT
...
int StructTranspose ( int n, int* column, int* row, int* &tColumn, int* &tRow ) {
    ...
    for ( i = 0; i < n; i++ ) {
        int j1 = row[i];
        int j2 = row[i + 1];
        int Col = i; // —толбец в AT - строка в ј
        
        for ( j = j1; j < j2; j++ ) {
            int RIndex = column[j];  // —трока в AT
...
* символическа€ фаза ILU(p) * INPUT * int n - размер матрицы * матрица A * int * col - индексы колонок матрицы a * int * row - индексы начала строк матрицы a
* портрет матрицы LU
* int * &lucol - индексы колонок матрицы lu
* int * &lurow - индексы начала строк матрицы lu * double * &luval - значени€ матрицы
* int * uptr - индексы диагональных элементов * в массиве luval
* OUTPUT
* double * luval - значени€ L и U разложенных матриц
* int &countL - размер матрицы L
* int &countU - размер матрицы U
* RETURN * возвращаетс€ код ошибки **/
